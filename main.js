...
function collision() {
  for (i = 0; i < entitys.length; i++) {
    if (entitys[i].name != "player") {
      if (
        entitys[i].posX - 15 < entity[playerIndex].posX &&
        entity[playerIndex].posX < entitys[i].posX + 15 &&
        entitys[i].posY - 15 < entity[playerIndex].posY &&
          entity[playerIndex].posY < entitys[i].posY + 15
      ) {
        gameOver();
      }
    }
